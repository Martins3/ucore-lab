...
static void stride_dequeue(struct run_queue *rq, struct proc_struct *proc) {
   ...
   */
#if USE_SKEW_HEAP
  assert(rq->lab6_run_pool != NULL && proc->rq == rq);
...
static struct proc_struct *stride_pick_next(struct run_queue *rq) {
   ...
   * (1) get a  proc_struct pointer p  with the minimum value of stride
   ...
   rq->lab6_run_pool
          (1.2) If using list, we have to search list to find the p
   with minimum stride value
   * (2) update p;
   *    s stride value: p->lab6_stride
   * (3) return p
   */
  ...
  while (le != &(rq->run_list)) {
    if (proc_stride_comp_f(le, target) == -1) {
      target = le;
    }
    le = list_next(le);
  }
